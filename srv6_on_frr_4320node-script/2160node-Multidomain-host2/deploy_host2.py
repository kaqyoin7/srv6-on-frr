#!/usr/bin/env python3
"""
SRv6+FRR deployment orchestrator for HOST 2 (rings 30-59).

Cross-host boundary handling (ring29 <-> ring30):
  - r30nx containers are started and configured normally.
  - For each cross-host link:
      * veth pair  r30nx-r29nx / r29nx-r30nx  is created on the HOST hypervisor.
      * The r30nx-r29nx end is moved into the srv6-r30nx container namespace
        and assigned its IPv6 address.
      * The r29nx-r30nx end is left on the hypervisor (no container move),
        to be bridged by your existing cross-host mechanism.
  - No containers are created for ring29 nodes on this host.
"""
...
class Host2SRv6Deployer:
    def __init__(self, topology_file, config_dir="configs"):
        ...
        self.base_network_name = self.topology.get("network_name", "srv6-host2-net")
        self.image_name    = "frr-srv6-node:latest"
        self.max_workers   = 30

        # Cross-host boundary metadata
        cross = self.topology.get("cross_host_boundary", {})
        self.cross_host_local_ring  = cross.get("local_ring",  30)   # ring30 on host2
        self.cross_host_remote_ring = cross.get("remote_ring", 29)   # ring29 on host1
        ...
        # Docker bridge networks: 5 bridges spanning rings 30-59
        self.num_networks  = 5
        self.network_names = [f"{self.base_network_name}-{i}" for i in range(self.num_networks)]
        ring_offset        = self.topology.get("ring_offset", 30)
        num_rings          = self.topology.get("num_rings", 30)
        rings_per_net      = max(1, num_rings // self.num_networks)
        self._node_network = {}
        for node in self.topology["nodes"]:
            local_ring_idx = node["ring"] - ring_offset          # 0..29
            net_idx        = min(local_ring_idx // rings_per_net, self.num_networks - 1)
    ...
    def cleanup(self):
        ...
        # Clean up hypervisor-side stubs (r29nx-r30nx that live on host2 hypervisor)
        ...
        for node_idx in range(self.topology["nodes_per_ring"]):
            stub_name = (
                f"r{self.cross_host_remote_ring}n{node_idx}"
                f"-r{self.cross_host_local_ring}n{node_idx}"
            )
    ...
    # ------------------------------------------------------------------
    # Step 3: Start containers  (only local nodes ring30-59, no ring29)
    # ------------------------------------------------------------------
    ...
    def start_containers(self):
        print("\n" + "=" * 60)
        print("Step 3: Start FRR containers (ring30-59 only)")
        print("=" * 60)
    ...
    #
    #  Topology (host2 perspective):
    #    veth pair:  r30nx-r29nx  (local end)  /  r29nx-r30nx  (hypervisor stub)
    #
    #  Actions:
    #    1. Create veth pair on hypervisor.
    #    2. Move r30nx-r29nx into the srv6-r30nx container namespace.
    #    3. Bring the interface up inside the container.
    #    4. Assign IPv6 address inside the container.
    #    5. Leave r29nx-r30nx on the hypervisor (bring up, no IP).
    #       Your bridging mechanism will connect it to host1's r29nx-r30nx stub.
    # ------------------------------------------------------------------
    ...
    def create_single_cross_host_link(self, link):
        ...
        In the host2 topology JSON:
          node1 = r29nx  -> deploy_node1 = "hypervisor"  -> stays on hypervisor
          node2 = r30nx  -> deploy_node2 = "container"   -> moved into container
        """
        node1  = link["node1"]   # r29nx  (remote / hypervisor stub on host2)
        node2  = link["node2"]   # r30nx  (local)
        subnet = link["subnet"]

        deploy_n1 = link.get("deploy_node1", "hypervisor")
        deploy_n2 = link.get("deploy_node2", "container")

        local_node  = node2 if deploy_n2 == "container" else node1
        remote_stub = node1 if deploy_n2 == "container" else node2
        ...
        # 4. Assign IPv6 address inside the container
        #    node1 always ::1, node2 always ::2 by topology convention
        if local_node == node1:
        ...
        # 5. Bring remote_veth (hypervisor stub) up – no IP assigned
    ...
    def create_links(self):
        ...
        # --- 5b: Cross-host links ---
        lr = self.cross_host_local_ring    # 30
        rr = self.cross_host_remote_ring   # 29
        print(f"\n  -> Cross-host boundary links (ring{rr} <-> ring{lr}): {cross_total} links")
        print(f"     Local end  (r{lr}nx-r{rr}nx) -> container")
        print(f"     Remote end (r{rr}nx-r{lr}nx) -> hypervisor stub")
    ...
    def verify_deployment(self):
        ...
        for node_idx in range(min(5, self.topology["nodes_per_ring"])):
            stub = f"r{rr}n{node_idx}-r{lr}n{node_idx}"
    ...
    def print_summary(self):
        print("\n" + "=" * 60)
        print("HOST2 DEPLOYMENT COMPLETED")
        print("=" * 60)
        nodes = self.topology["nodes"]
        lr    = self.cross_host_local_ring
        rr    = self.cross_host_remote_ring
        print(f"Nodes deployed : {len(nodes)} (rings {lr}-59)")
...
Cross-host stub interfaces on this hypervisor:
  ...
  -> Connect these to your bridge / VXLAN / GRE toward host1.
...
Cleanup:
  sudo python3 deploy_host2.py --cleanup <topology_file>
""")
    ...
    def deploy(self):
        print("\n" + "=" * 60)
        print("SRv6+FRR HOST2 DEPLOYMENT (rings 30-59, 2160 nodes)")
        print("=" * 60)
        print(f"Nodes           : {len(self.topology['nodes'])}")
        print(f"Normal links    : {len(self.normal_links)}")
        print(f"Cross-host links: {len(self.cross_host_links)} (ring{self.cross_host_remote_ring} <-> ring{self.cross_host_local_ring})")
...
def main():
    parser = argparse.ArgumentParser(description="Deploy Host2 SRv6+FRR network (rings 30-59)")
    parser.add_argument("topology", nargs="?",
                        default="topology_host2_2160nodes.json",
    ...
    deployer = Host2SRv6Deployer(args.topology, args.config_dir)
